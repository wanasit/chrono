...
var moment=require("moment"),Parser=require("../parser").Parser,ParsedResult=require("../../result").ParsedResult,PATTERN=/(\W|^)((утром|обед|полдень|вечером|полночь|ночью|ночь))/i,TIME_MATCH=4;exports.Parser=function(){Parser.apply(this,arguments),this.pattern=function(){return PATTERN},this.extract=function(r,e,t,s){var r=t[0].substr(t[1].length),a=t.index+t[1].length,i=new ParsedResult({index:a,text:r,ref:e});switch(t[TIME_MATCH]||(TIME_MATCH=3),t[TIME_MATCH].toLowerCase()){case"обед":i.start.imply("meridiem",1),i.start.imply("hour",15);break;case"вечером":case"ночью":i.start.imply("meridiem",1),i.start.imply("hour",22);break;case"утром":i.start.imply("meridiem",0),i.start.imply("hour",6);break;case"полдень":i.start.imply("meridiem",0),i.start.imply("hour",12);break;case"полночь":case"ночь":i.start.imply("meridiem",1),i.start.imply("hour",24)}return i.tags.RUCasualTimeParser=!0,i}};
