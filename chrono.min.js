...
var moment=require("moment"),Parser=require("../parser").Parser,ParsedResult=require("../../result").ParsedResult,util=require("../../utils/EN"),PATTERN=new RegExp("(\\W|^)(within|in)\\s*("+util.INTEGER_WORDS_PATTERN+"|[0-9]+|an?|half(?:\\s*an?)?)\\s*(seconds?|minutes?|hours?|days?)\\s*(?=\\W|$)","i");exports.Parser=function(){Parser.apply(this,arguments),this.pattern=function(){return PATTERN},this.extract=function(t,a,e,r){var s=e.index+e[1].length,t=e[0];t=e[0].substr(e[1].length,e[0].length-e[1].length);var n=new ParsedResult({index:s,text:t,ref:a}),i=e[3];i=void 0!==util.INTEGER_WORDS[i]?5:"a"===i||"an"===i?1:i.match(/half/)?.5:parseInt(i);var u=moment(a);return e[4].match(/day/)?(u.add(i,"d"),n.start.assign("year",u.year()),n.start.assign("month",u.month()+1),n.start.assign("day",u.date()),n):(e[4].match(/hour/)?u.add(i,"hour"):e[4].match(/minute/)?u.add(i,"minute"):e[4].match(/second/)&&u.add(i,"second"),n.start.imply("year",u.year()),n.start.imply("month",u.month()+1),n.start.imply("day",u.date()),n.start.assign("hour",u.hour()),n.start.assign("minute",u.minute()),n.start.assign("second",u.second()),n.tags.ENDeadlineFormatParser=!0,n)}};
...
var moment=require("moment"),Parser=require("../parser").Parser,ParsedResult=require("../../result").ParsedResult,util=require("../../utils/EN"),PATTERN=new RegExp("(\\W|^)(?:within\\s*)?("+util.INTEGER_WORDS_PATTERN+"|[0-9]+|an?|half(?:\\s*an?)?)\\s*(seconds?|minutes?|hours?|weeks?|days?|months?|years?)\\s*(?:ago|before|earlier)(?=(?:\\W|$))","i"),STRICT_PATTERN=new RegExp("(\\W|^)(?:within\\s*)?([0-9]+|an?)\\s*(seconds?|minutes?|hours?|days?)\\s*ago(?=(?:\\W|$))","i");exports.Parser=function(){Parser.apply(this,arguments),this.pattern=function(){return this.isStrictMode()?STRICT_PATTERN:PATTERN},this.extract=function(t,e,a,r){if(a.index>0&&t[a.index-1].match(/\w/))return null;var t=a[0];t=a[0].substr(a[1].length,a[0].length-a[1].length),index=a.index+a[1].length;var s=new ParsedResult({index:index,text:t,ref:e}),n=a[2];n=void 0!==util.INTEGER_WORDS[n]?util.INTEGER_WORDS[n]:"a"===n||"an"===n?1:n.match(/half/)?.5:parseInt(n);var i=moment(e);return a[3].match(/hour|minute|second/)?(a[3].match(/hour/)?i.add(-n,"hour"):a[3].match(/minute/)?i.add(-n,"minute"):a[3].match(/second/)&&i.add(-n,"second"),s.start.imply("day",i.date()),s.start.imply("month",i.month()+1),s.start.imply("year",i.year()),s.start.assign("hour",i.hour()),s.start.assign("minute",i.minute()),s.start.assign("second",i.second()),s.tags.ENTimeAgoFormatParser=!0,s):a[3].match(/week/)?(i.add(-n,"week"),s.start.imply("day",i.date()),s.start.imply("month",i.month()+1),s.start.imply("year",i.year()),s.start.imply("weekday",i.day()),s):(a[3].match(/day/)&&i.add(-n,"d"),a[3].match(/month/)&&i.add(-n,"month"),a[3].match(/year/)&&i.add(-n,"year"),s.start.assign("day",i.date()),s.start.assign("month",i.month()+1),s.start.assign("year",i.year()),s)}};
