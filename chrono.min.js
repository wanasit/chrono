...
var moment=require("moment"),Parser=require("../parser").Parser,ParsedResult=require("../../result").ParsedResult,ParsedComponents=require("../../result").ParsedComponents,FIRST_REG_PATTERN=new RegExp("(^|\\s|T)(?:(?:at|from)\\s*)??(\\d{1,4}|noon|midnight)(?:(?:\\.|\\:|\\：)(\\d{1,2})(?:(?:\\:|\\：)(\\d{2})(?:\\.(\\d{1,3}))?)?)?(?:\\s*(A\\.M\\.|P\\.M\\.|AM?|PM?|O\\W*CLOCK))?(?=\\W|$)","i"),SECOND_REG_PATTERN=new RegExp("^\\s*(\\-|\\–|\\~|\\〜|to|\\?)\\s*(\\d{1,4})(?:(?:\\.|\\:|\\：)(\\d{1,2})(?:(?:\\.|\\:|\\：)(\\d{1,2})(?:\\.(\\d{1,4}))?)?)?(?:\\s*(A\\.M\\.|P\\.M\\.|AM?|PM?|O\\W*CLOCK))?(?=\\W|$)","i"),HOUR_GROUP=2,MINUTE_GROUP=3,SECOND_GROUP=4,MILLI_SECOND_GROUP=5,AM_PM_HOUR_GROUP=6;exports.Parser=function(){Parser.apply(this,arguments),this.pattern=function(){return FIRST_REG_PATTERN},this.extract=function(e,t,r,n){if(r.index>0&&e[r.index-1].match(/\w/))return null;var s=moment(t),i=new ParsedResult;i.ref=t,i.index=r.index+r[1].length,i.text=r[0].substring(r[1].length),i.tags.ENTimeExpressionParser=!0,i.start.imply("day",s.date()),i.start.imply("month",s.month()+1),i.start.imply("year",s.year());var a=0,l=0,d=-1;if(null!=r[MILLI_SECOND_GROUP]){var u=parseInt(r[MILLI_SECOND_GROUP]);if(u>=1e3)return null;i.start.assign("millisecond",u)}if(null!=r[SECOND_GROUP]){var m=parseInt(r[SECOND_GROUP]);if(m>=60)return null;i.start.assign("second",m)}if("noon"==r[HOUR_GROUP].toLowerCase()?(d=1,a=12):"midnight"==r[HOUR_GROUP].toLowerCase()?(d=0,a=0):a=parseInt(r[HOUR_GROUP]),null!=r[MINUTE_GROUP]?l=parseInt(r[MINUTE_GROUP]):a>100&&(l=a%100,a=parseInt(a/100)),l>=60)return null;if(a>24)return null;if(a>=12&&(d=1),null!=r[AM_PM_HOUR_GROUP]){if(a>12)return null;var O=r[AM_PM_HOUR_GROUP][0].toLowerCase();"a"==O&&(d=0,12==a&&(a=0)),"p"==O&&(d=1,12!=a&&(a+=12))}if(i.start.assign("hour",a),i.start.assign("minute",l),d>=0?i.start.assign("meridiem",d):a<12?i.start.imply("meridiem",0):i.start.imply("meridiem",1),!(r=SECOND_REG_PATTERN.exec(e.substring(i.index+i.text.length))))return i.text.match(/^\d+$/)?null:i;if(r[0].match(/^\s*(\+|\-)\s*\d{3,4}$/))return i;null==i.end&&(i.end=new ParsedComponents(null,i.start.date()));var a=0,l=0,d=-1;if(null!=r[MILLI_SECOND_GROUP]){var u=parseInt(r[MILLI_SECOND_GROUP]);if(u>=1e3)return null;i.end.assign("millisecond",u)}if(null!=r[SECOND_GROUP]){var m=parseInt(r[SECOND_GROUP]);if(m>=60)return null;i.end.assign("second",m)}if(a=parseInt(r[2]),null!=r[MINUTE_GROUP]){if((l=parseInt(r[MINUTE_GROUP]))>=60)return i}else a>100&&(l=a%100,a=parseInt(a/100));if(l>=60)return null;if(a>24)return null;if(a>=12&&(d=1),null!=r[AM_PM_HOUR_GROUP]){if(a>12)return null;var O=r[AM_PM_HOUR_GROUP][0].toLowerCase();"a"==O&&(d=0,12==a&&(a=0,i.end.isCertain("day")||i.end.imply("day",i.end.get("day")+1))),"p"==O&&(d=1,12!=a&&(a+=12)),i.start.isCertain("meridiem")||(0==d?(i.start.imply("meridiem",0),12==i.start.get("hour")&&i.start.assign("hour",0)):(i.start.imply("meridiem",1),12!=i.start.get("hour")&&i.start.assign("hour",i.start.get("hour")+12)))}if(i.text=i.text+r[0],i.end.assign("hour",a),i.end.assign("minute",l),d>=0)i.end.assign("meridiem",d);else{i.start.isCertain("meridiem")&&1==i.start.get("meridiem")&&i.start.get("hour")>a?i.end.imply("meridiem",0):a>12&&i.end.imply("meridiem",1)}return i.end.date().getTime()<i.start.date().getTime()&&i.end.imply("day",i.end.get("day")+1),i}};
