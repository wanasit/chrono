...
var moment=require("moment"),Parser=require("../parser").Parser,ParsedResult=require("../../result").ParsedResult,PATTERN=new RegExp("(\\W|^)((?:sun|mon|tues?|wed(?:nes)?|thu(?:rs?)?|fri|sat(?:ur)?)(?:day)?)?\\s*\\,?\\s*([0-9]{1,2})[\\/\\.\\-]([0-9]{1,2})([\\/\\.\\-]([0-9]{4}|[0-9]{2}))?(\\W|$)","i"),DAYS_OFFSET={sunday:0,sun:0,monday:1,mon:1,tuesday:2,wednesday:3,wed:3,thursday:4,thur:4,friday:5,fri:5,saturday:6,sat:6};exports.Parser=function(){Parser.call(this),this.pattern=function(){return PATTERN},this.extract=function(e,t,r){if("/"==r[1]||"/"==r[7])return void(r.index+=r[0].length);var a=r.index+r[1].length,e=r[0].substr(r[1].length,r[0].length-r[7].length),s=new ParsedResult({text:e,index:a,ref:t});if(!(e.match(/^\d.\d$/)||!r[6]&&r[0].indexOf("/")<0)){var n=null,u=r[6]||moment(t).year()+"",i=r[3],d=r[4];if(i=parseInt(i),d=parseInt(d),u=parseInt(u),(1>i||i>12)&&i>12){if(!(d>=1&&12>=d&&i>=13&&31>=i))return null;var l=i;i=d,d=l}return 1>d||d>31?null:(100>u&&(u>50?u=u+2500-543:u+=2e3),e=i+"/"+d+"/"+u,n=moment(e,"M/D/YYYY"),n&&n.date()==d&&n.month()==i-1?(s.start.assign("day",n.date()),s.start.assign("month",n.month()+1),s.start.assign("year",n.year()),r[2]&&s.start.assign("weekday",DAYS_OFFSET[r[2].toLowerCase()]),s.tags.ENSlashDateFormatParser=!0,s):null)}}};
},{"../../result":25,"../parser":16,"moment":1}],10:[function(require,module,exports){
var moment=require("moment"),Parser=require("../parser").Parser,ParsedResult=require("../../result").ParsedResult,PATTERN=new RegExp("(\\W|^)([0-9]{4})[\\-\\.\\/]([0-9]{1,2})[\\-\\.\\/]([0-9]{1,2})(?=\\W|$)","i"),YEAR_NUMBER_GROUP=2,MONTH_NUMBER_GROUP=3,DATE_NUMBER_GROUP=4;exports.Parser=function(){Parser.call(this),this.pattern=function(){return PATTERN},this.extract=function(t,e,r){var t=r[0].substr(r[1].length),s=r.index+r[1].length,a=new ParsedResult({text:t,index:s,ref:e});return a.start.assign("year",parseInt(r[YEAR_NUMBER_GROUP])),a.start.assign("month",parseInt(r[MONTH_NUMBER_GROUP])),a.start.assign("day",parseInt(r[DATE_NUMBER_GROUP])),moment(a.start.get("month"))>12||moment(a.start.get("month"))<1||moment(a.start.get("day"))>31||moment(a.start.get("day"))<1?null:(a.tags.ENDateFormatParser=!0,a)}};
...
var moment=require("moment"),Parser=require("../parser").Parser,ParsedResult=require("../../result").ParsedResult,PATTERN=new RegExp("(\\W|^)(?:(?:\\,|\\(|\\（)\\s*)?(?:(this|last|next)\\s*)?(Sunday|Sun|Monday|Mon|Tuesday|Tues|Tue|Wednesday|Wed|Thursday|Thurs|Thur|Friday|Fri|Saturday|Sat)(?:\\s*(?:\\,|\\)|\\）))?(?:\\s*(this|last|next)\\s*week)?(?=\\W|$)","i"),DAYS_OFFSET={sunday:0,sun:0,monday:1,mon:1,tuesday:2,tues:2,tue:2,wednesday:3,wed:3,thursday:4,thurs:4,thur:4,thu:4,friday:5,fri:5,saturday:6,sat:6},PREFIX_GROUP=2,WEEKDAY_GROUP=3,POSTFIX_GROUP=4;exports.Parser=function(){Parser.call(this),this.pattern=function(){return PATTERN},this.extract=function(e,t,a){var r=a.index+a[1].length,e=a[0].substr(a[1].length,a[0].length-a[1].length),s=new ParsedResult({index:r,text:e,ref:t}),n=a[WEEKDAY_GROUP].toLowerCase(),u=DAYS_OFFSET[n];if(void 0===u)return null;var d=moment(t),i=a[PREFIX_GROUP],y=a[POSTFIX_GROUP];if(i||y){var h=i||y;h=h.toLowerCase(),"last"==h?d.day(u-7):"next"==h?d.day(u+7):"this"==h&&d.day(u)}else{var l=d.day();d.day(Math.abs(u-7-l)<Math.abs(u-l)?u-7:Math.abs(u+7-l)<Math.abs(u-l)?u+7:u)}return s.start.assign("weekday",u),s.start.imply("day",d.date()),s.start.imply("month",d.month()+1),s.start.imply("year",d.year()),s}};
