...
var moment=require("moment"),Parser=require("../parser").Parser,ParsedResult=require("../../result").ParsedResult,util=require("../../utils/EN"),PATTERN=new RegExp("(^|\\D\\s+)(Jan\\.?|January|Feb\\.?|February|Mar\\.?|March|Apr\\.?|April|May\\.?|Jun\\.?|June|Jul\\.?|July|Aug\\.?|August|Sep\\.?|Sept\\.?|September|Oct\\.?|October|Nov\\.?|November|Dec\\.?|December)\\s*(?:,?\\s*([0-9]{4})(\\s*BE)?)?(?=[^\\s\\w]|$)","i"),MONTH_NAME_GROUP=2,YEAR_GROUP=3,YEAR_BE_GROUP=4;exports.Parser=function(){Parser.apply(this,arguments),this.pattern=function(){return PATTERN},this.extract=function(e,t,r,a){var s=new ParsedResult({text:r[0].substr(r[1].length,r[0].length-r[1].length),index:r.index+r[1].length,ref:t}),n=r[MONTH_NAME_GROUP];n=util.MONTH_OFFSET[n.toLowerCase()];var i=1,u=null;if(r[YEAR_GROUP]&&(u=r[YEAR_GROUP],u=parseInt(u),r[YEAR_BE_GROUP]?u-=543:100>u&&(u+=2e3)),u)s.start.assign("day",i),s.start.assign("month",n),s.start.assign("year",u);else{var m=moment(t);m.month(n-1),m.date(i);var o=m.clone().add(1,"y"),l=m.clone().add(-1,"y");Math.abs(o.diff(moment(t)))<Math.abs(m.diff(moment(t)))?m=o:Math.abs(l.diff(moment(t)))<Math.abs(m.diff(moment(t)))&&(m=l),s.start.assign("day",i),s.start.assign("month",n),s.start.imply("year",m.year())}return s.tags.ENMonthNameYearParser=!0,s}};
...
var moment=require("moment"),Parser=require("../parser").Parser,ParsedResult=require("../../result").ParsedResult,PATTERN=new RegExp("(^|[^\\d/]\\s+)([0-9]|0[1-9]|1[012])/([0-9]{4})(\\s+[^\\d/]|$)","i"),OPENNING_GROUP=1,ENDING_GROUP=4,MONTH_GROUP=2,YEAR_GROUP=3;exports.Parser=function(e){Parser.apply(this,arguments),this.pattern=function(){return PATTERN},this.extract=function(e,r,t,s){var a=t.index+t[OPENNING_GROUP].length,e=t[0].substr(t[OPENNING_GROUP].length,t[0].length-t[ENDING_GROUP].length),n=new ParsedResult({text:e,index:a,ref:r}),P=t[YEAR_GROUP],N=t[MONTH_GROUP],R=1;return N=parseInt(N),P=parseInt(P),n.start.assign("day",R),n.start.assign("month",N),n.start.assign("year",P),n.tags.ENSlashMonthYearFormatParser=!0,n}};
