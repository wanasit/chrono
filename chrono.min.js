...
var moment=require("moment"),Parser=require("../parser").Parser,ParsedResult=require("../../result").ParsedResult,PATTERN=/(\W|^)(within|in)\s*([0-9]+|an?|half(?:\s*an?)?)\s*(seconds?|minutes?|hours?|days?)\s*(?=(?:\W|$))/i;exports.Parser=function(){Parser.apply(this,arguments),this.pattern=function(){return PATTERN},this.extract=function(t,a,e,r){var s=e.index+e[1].length,t=e[0];t=e[0].substr(e[1].length,e[0].length-e[1].length);var n=new ParsedResult({index:s,text:t,ref:a}),i=e[3];i="a"===i||"an"===i?1:i.match(/half/)?.5:parseInt(i);var d=moment(a);return e[4].match(/day/)?(d.add(i,"d"),n.start.assign("year",d.year()),n.start.assign("month",d.month()+1),n.start.assign("day",d.date()),n):(e[4].match(/hour/)?d.add(i,"hour"):e[4].match(/minute/)?d.add(i,"minute"):e[4].match(/second/)&&d.add(i,"second"),n.start.imply("year",d.year()),n.start.imply("month",d.month()+1),n.start.imply("day",d.date()),n.start.assign("hour",d.hour()),n.start.assign("minute",d.minute()),n.start.assign("second",d.second()),n.tags.ENDeadlineFormatParser=!0,n)}};
...
var moment=require("moment"),Parser=require("../parser").Parser,ParsedResult=require("../../result").ParsedResult,PATTERN=/(\W|^)(?:within\s*)?([0-9]+|an?|half(?:\s*an?)?)\s*(seconds?|minutes?|hours?|weeks?|days?|months?|years?)\s*(?:ago|before|earlier)(?=(?:\W|$))/i,STRICT_PATTERN=/(\W|^)(?:within\s*)?([0-9]+|an?)\s*(seconds?|minutes?|hours?|days?)\s*ago(?=(?:\W|$))/i;exports.Parser=function(){Parser.apply(this,arguments),this.pattern=function(){return this.isStrictMode()?STRICT_PATTERN:PATTERN},this.extract=function(t,a,e,s){if(e.index>0&&t[e.index-1].match(/\w/))return null;var t=e[0];t=e[0].substr(e[1].length,e[0].length-e[1].length),index=e.index+e[1].length;var r=new ParsedResult({index:index,text:t,ref:a}),n=e[2];n="a"===n||"an"===n?1:n.match(/half/)?.5:parseInt(n);var i=moment(a);return e[3].match(/hour|minute|second/)?(e[3].match(/hour/)?i.add(-n,"hour"):e[3].match(/minute/)?i.add(-n,"minute"):e[3].match(/second/)&&i.add(-n,"second"),r.start.imply("day",i.date()),r.start.imply("month",i.month()+1),r.start.imply("year",i.year()),r.start.assign("hour",i.hour()),r.start.assign("minute",i.minute()),r.start.assign("second",i.second()),r.tags.ENTimeAgoFormatParser=!0,r):e[3].match(/week/)?(i.add(-n,"week"),r.start.imply("day",i.date()),r.start.imply("month",i.month()+1),r.start.imply("year",i.year()),r.start.imply("weekday",i.day()),r):(e[3].match(/day/)&&i.add(-n,"d"),e[3].match(/month/)&&i.add(-n,"month"),e[3].match(/year/)&&i.add(-n,"year"),r.start.assign("day",i.date()),r.start.assign("month",i.month()+1),r.start.assign("year",i.year()),r)}};
