...
var moment=require("moment"),Parser=require("../parser").Parser,ParsedResult=require("../../result").ParsedResult,PATTERN=/(\W|^)(maintenant|aujourd'hui|demain|hier|nuit dernière|soir|(ce|cet)\s*(matin|après\-midi|aprèm|soir))(?=\W|$)/i;exports.Parser=function(){Parser.apply(this,arguments),this.pattern=function(){return PATTERN},this.extract=function(r,e,t,a){var r=t[0].substr(t[1].length),s=t.index+t[1].length,i=new ParsedResult({index:s,text:r,ref:e}),n=moment(e),m=n.clone(),o=r.toLowerCase();if("soir"==o)i.start.imply("hour",22),i.start.imply("meridiem",1);else if("demain"==o)n.hour()>1&&m.add(1,"day");else if("hier"==o)m.add(-1,"day");else if(o.match(/nuit dernière/))i.start.imply("hour",0),n.hour()>6&&m.add(-1,"day");else if(o.match("ce")){var u=t[4].toLowerCase();"après-midi"==u||"aprèm"==u?i.start.imply("hour",15):"soir"==u?i.start.imply("hour",18):"matin"==u&&i.start.imply("hour",6)}else o.match("maintenant")&&(i.start.imply("hour",n.hour()),i.start.imply("minute",n.minute()),i.start.imply("second",n.second()));return i.start.assign("day",m.date()),i.start.assign("month",m.month()+1),i.start.assign("year",m.year()),i.tags.ENCasualDateParser=!0,i}};
},{"../../result":37,"../parser":28,"moment":1}],22:[function(require,module,exports){
var moment=require("moment"),Parser=require("../parser").Parser,ParsedResult=require("../../result").ParsedResult,util=require("../../utils/FR"),DAYS_OFFSET={dimanche:0,dim:0,lundi:1,lun:1,mardi:2,mar:2,mercredi:3,mer:3,jeudi:4,jeu:4,vendredi:5,ven:5,samedi:6,sam:6},PATTERN=new RegExp("(\\W|^)(?:(Dimanche|Lundi|Mardi|Mercredi|Jeudi|Vendredi|Samedi|Dim|Lun|Mar|Mer|Jeu|Ven|Sam)\\s*,?\\s*)?([0-9]{1,2})(?:st|nd|rd|th)?(?:\\s*(?:to|\\-|\\–|until|through|till|\\s)\\s*([0-9]{1,2})(?:st|nd|rd|th)?)?\\s*(?:of)?\\s*(Jan(?:vier|\\.)?|F(é|e)v(?:rier|\\.)?|Mar(?:s|\\.)?|Avr(?:il|\\.)?|Mai|Juin|Juillet|Aoû(?:t|\\.)?|Sep(?:tembre|\\.)?|Oct(?:obre|\\.)?|Nov(?:embre|\\.)?|D(é|e)c(?:embre|\\.)?)(?:(\\s*[0-9]{2,4}(?![^\\s]\\d))(\\s*BE)?)?(?=\\W|$)","i"),WEEKDAY_GROUP=2,DATE_GROUP=3,DATE_TO_GROUP=4,MONTH_NAME_GROUP=5,YEAR_GROUP=6,YEAR_BE_GROUP=7;exports.Parser=function(){Parser.apply(this,arguments),this.pattern=function(){return PATTERN},this.extract=function(e,r,t,a){var s=new ParsedResult({text:t[0].substr(t[1].length,t[0].length-t[1].length),index:t.index+t[1].length,ref:r}),n=t[MONTH_NAME_GROUP];n=util.MONTH_OFFSET[n.toLowerCase()];var i=t[DATE_GROUP];i=parseInt(i);var d=null;if(t[YEAR_GROUP]&&(d=t[YEAR_GROUP],d=parseInt(d),t[YEAR_BE_GROUP]?d-=543:100>d&&(d+=2e3)),d)s.start.assign("day",i),s.start.assign("month",n),s.start.assign("year",d);else{var m=moment(r);m.month(n-1),m.date(i),m.year(moment(r).year());var u=m.clone().add(1,"y"),E=m.clone().add(-1,"y");Math.abs(u.diff(moment(r)))<Math.abs(m.diff(moment(r)))?m=u:Math.abs(E.diff(moment(r)))<Math.abs(m.diff(moment(r)))&&(m=E),s.start.assign("day",i),s.start.assign("month",n),s.start.imply("year",m.year())}if(t[WEEKDAY_GROUP]){var l=t[WEEKDAY_GROUP];l=util.WEEKDAY_OFFSET[l.toLowerCase()],s.start.assign("weekday",l)}return t[DATE_TO_GROUP]&&(s.end=s.start.clone(),s.end.assign("day",parseInt(t[DATE_TO_GROUP]))),s.tags.FRMonthNameLittleEndianParser=!0,s}};
},{"../../result":37,"../../utils/FR":40,"../parser":28,"moment":1}],23:[function(require,module,exports){
var moment=require("moment"),Parser=require("../parser").Parser,ParsedResult=require("../../result").ParsedResult,util=require("../../utils/FR"),DAYS_OFFSET={dimanche:0,dim:0,lundi:1,lun:1,mardi:2,mar:2,mercredi:3,mer:3,jeudi:4,jeu:4,vendredi:5,ven:5,samedi:6,sam:6},PATTERN=new RegExp("(\\W|^)(?:(Dimanche|Lundi|Mardi|Mercredi|Jeudi|Vendredi|Samedi|Dim\\.?|Lun\\.?|Mar\\.?|Mer\\.?|Jeu\\.?|Ven\\.?|Sam\\.?)\\s*,?\\s*)?(Jan\\.?|Janvier|Fev\\.?|Fév\\.?|Février|Mar\\.?|Mars|Avr\\.?|Avril|Mai\\.?|Juin|Juillet|Aoû\\.?|Août|Sep\\.?|Sept\\.?|Septembre|Oct\\.?|Octobre|Nov\\.?|Novembre|Dec\\.?|Déc\\.?|Décembre)\\s*([0-9]{1,2})(?:st|nd|rd|th)?\\s*(?:(?:to|\\-)\\s*([0-9]{1,2})(?:st|nd|rd|th)?\\s*)?(?:\\s*,?\\s*(s*[0-9]{4})(\\s*BE)?\\s*)?(?=\\W|$)","i"),WEEKDAY_GROUP=2,MONTH_NAME_GROUP=3,DATE_GROUP=4,DATE_TO_GROUP=5,YEAR_GROUP=6,YEAR_BE_GROUP=7;exports.Parser=function(){Parser.apply(this,arguments),this.pattern=function(){return PATTERN},this.extract=function(e,r,t,a){var s=new ParsedResult({text:t[0].substr(t[1].length,t[0].length-t[1].length),index:t.index+t[1].length,ref:r}),n=t[MONTH_NAME_GROUP];n=util.MONTH_OFFSET[n.toLowerCase()];var i=t[DATE_GROUP];i=parseInt(i);var d=null;if(t[YEAR_GROUP]&&(d=t[YEAR_GROUP],d=parseInt(d),t[YEAR_BE_GROUP]?d-=543:100>d&&(d+=2e3)),d)s.start.assign("day",i),s.start.assign("month",n),s.start.assign("year",d);else{var m=moment(r);m.month(n-1),m.date(i);var E=m.clone().add(1,"y"),u=m.clone().add(-1,"y");Math.abs(E.diff(moment(r)))<Math.abs(m.diff(moment(r)))?m=E:Math.abs(u.diff(moment(r)))<Math.abs(m.diff(moment(r)))&&(m=u),s.start.assign("day",i),s.start.assign("month",n),s.start.imply("year",m.year())}if(t[WEEKDAY_GROUP]){var o=t[WEEKDAY_GROUP];o=util.WEEKDAY_OFFSET[o.toLowerCase()],s.start.assign("weekday",o)}return t[DATE_TO_GROUP]&&(s.end=s.start.clone(),s.end.assign("day",parseInt(t[DATE_TO_GROUP]))),s.tags.FRMonthNameMiddleEndianParser=!0,s}};
},{"../../result":37,"../../utils/FR":40,"../parser":28,"moment":1}],24:[function(require,module,exports){
var moment=require("moment"),Parser=require("../parser").Parser,ParsedResult=require("../../result").ParsedResult,ParsedComponents=require("../../result").ParsedComponents,FIRST_REG_PATTERN=new RegExp("(^|\\s|T)(?:(?:à|de)\\s*)?(\\d{1,4}|midi|minuit)(?:(?:\\.|\\:|\\：|h)(\\d{1,2})(?:(?:\\:|\\：)(\\d{2}))?)?(?:\\s*(A\\.M\\.|P\\.M\\.|AM?|PM?))?(?=\\W|$)","i"),SECOND_REG_PATTERN=new RegExp("^\\s*(\\-|\\–|\\~|\\〜|à|\\?)\\s*(\\d{1,4})(?:(?:\\.|\\:|\\：|h)(\\d{1,2})(?:(?:\\.|\\:|\\：)(\\d{1,2}))?)?(?:\\s*(A\\.M\\.|P\\.M\\.|AM?|PM?))?(?=\\W|$)","i"),HOUR_GROUP=2,MINUTE_GROUP=3,SECOND_GROUP=4,AM_PM_HOUR_GROUP=5;exports.Parser=function(){Parser.apply(this,arguments),this.pattern=function(){return FIRST_REG_PATTERN},this.extract=function(e,t,r,n){if(r.index>0&&e[r.index-1].match(/\w/))return null;var s=moment(t),i=new ParsedResult;i.ref=t,i.index=r.index+r[1].length,i.text=r[0].substring(r[1].length),i.tags.FRTimeExpressionParser=!0,i.start.imply("day",s.date()),i.start.imply("month",s.month()+1),i.start.imply("year",s.year());var a=0,d=0,l=-1;if(null!=r[SECOND_GROUP]){var u=parseInt(r[SECOND_GROUP]);if(u>=60)return null;i.start.assign("second",u)}if("midi"==r[HOUR_GROUP].toLowerCase()?(l=1,a=12):"minuit"==r[HOUR_GROUP].toLowerCase()?(l=0,a=0):a=parseInt(r[HOUR_GROUP]),null!=r[MINUTE_GROUP]?d=parseInt(r[MINUTE_GROUP]):a>100&&(d=a%100,a=parseInt(a/100)),d>=60)return null;if(a>24)return null;if(a>=12&&(l=1),null!=r[AM_PM_HOUR_GROUP]){if(a>12)return null;var m=r[AM_PM_HOUR_GROUP][0].toLowerCase();"a"==m&&(l=0,12==a&&(a=0)),"p"==m&&(l=1,12!=a&&(a+=12))}if(i.start.assign("hour",a),i.start.assign("minute",d),l>=0?i.start.assign("meridiem",l):12>a?i.start.imply("meridiem",0):i.start.imply("meridiem",1),r=SECOND_REG_PATTERN.exec(e.substring(i.index+i.text.length)),!r)return i.text.match(/^\d+$/)?null:i;if(r[0].match(/^\s*(\+|\-)\s*\d{3,4}$/))return i;null==i.end&&(i.end=new ParsedComponents(null,i.start.date()));var a=0,d=0,l=-1;if(null!=r[SECOND_GROUP]){var u=parseInt(r[SECOND_GROUP]);if(u>=60)return null;i.end.assign("second",u)}if(a=parseInt(r[2]),null!=r[MINUTE_GROUP]){if(d=parseInt(r[MINUTE_GROUP]),d>=60)return i}else a>100&&(d=a%100,a=parseInt(a/100));if(d>=60)return null;if(a>24)return null;if(a>=12&&(l=1),null!=r[AM_PM_HOUR_GROUP]){if(a>12)return null;"a"==r[AM_PM_HOUR_GROUP][0].toLowerCase()&&(l=0,12==a&&(a=0,i.end.isCertain("day")||i.end.imply("day",i.end.get("day")+1))),"p"==r[AM_PM_HOUR_GROUP][0].toLowerCase()&&(l=1,12!=a&&(a+=12)),i.start.isCertain("meridiem")||(0==l?(i.start.imply("meridiem",0),12==i.start.get("hour")&&i.start.assign("hour",0)):(i.start.imply("meridiem",1),12!=i.start.get("hour")&&i.start.assign("hour",i.start.get("hour")+12)))}if(i.text=i.text+r[0],i.end.assign("hour",a),i.end.assign("minute",d),l>=0)i.end.assign("meridiem",l);else{var R=i.start.isCertain("meridiem")&&1==i.start.get("meridiem");R&&i.start.get("hour")>a?i.end.imply("meridiem",0):a>12&&i.end.imply("meridiem",1)}return i.end.date().getTime()<i.start.date().getTime()&&i.end.imply("day",i.end.get("day")+1),i}};
},{"../../result":37,"../parser":28,"moment":1}],25:[function(require,module,exports){
var moment=require("moment"),Parser=require("../parser").Parser,ParsedResult=require("../../result").ParsedResult,DAYS_OFFSET={dimanche:0,dim:0,lundi:1,lun:1,mardi:2,mar:2,mercredi:3,mer:3,jeudi:4,jeu:4,vendredi:5,ven:5,samedi:6,sam:6},PATTERN=new RegExp("(\\W|^)(?:(?:\\,|\\(|\\（)\\s*)?(?:(ce|dernier|prochain)\\s*)?("+Object.keys(DAYS_OFFSET).join("|")+")(?:\\s*(?:\\,|\\)|\\）))?(?:\\s*(ce|dernier|prochain)\\s*week)?(?=\\W|$)","i"),PREFIX_GROUP=2,WEEKDAY_GROUP=3,POSTFIX_GROUP=4;exports.Parser=function(){Parser.apply(this,arguments),this.pattern=function(){return PATTERN},this.extract=function(e,r,a,t){var n=a.index+a[1].length,e=a[0].substr(a[1].length,a[0].length-a[1].length),s=new ParsedResult({index:n,text:e,ref:r}),i=a[WEEKDAY_GROUP].toLowerCase(),d=DAYS_OFFSET[i];if(void 0===d)return null;var o=moment(r),l=a[PREFIX_GROUP],y=a[POSTFIX_GROUP];if(l||y){var m=l||y;m=m.toLowerCase(),"dernier"==m||"dernière"==m?o.day(d-7):"prochain"==m||"prochaine"==m?o.day(d+7):"ce"!=m&&"cet"!=m&&"cette"!=m||o.day(d)}else{var u=o.day();t.forwardDatesOnly&&u>d?o.day(d+7):!t.forwardDatesOnly&&Math.abs(d-7-u)<Math.abs(d-u)?o.day(d-7):!t.forwardDatesOnly&&Math.abs(d+7-u)<Math.abs(d-u)?o.day(d+7):o.day(d)}return s.start.assign("weekday",d),s.start.imply("day",o.date()),s.start.imply("month",o.month()+1),s.start.imply("year",o.year()),s}};
...
exports.WEEKDAY_OFFSET={dimanche:0,dim:0,lundi:1,lun:1,mardi:2,mar:2,mercredi:3,mer:3,jeudi:4,jeu:4,vendredi:5,ven:5,samedi:6,sam:6},exports.MONTH_OFFSET={janvier:1,jan:1,"jan.":1,"février":2,fev:2,"fev.":2,"fév":2,"fév.":2,mars:3,mar:3,"mar.":3,avril:4,avr:4,"avr.":4,mai:5,juin:6,juillet:7,"août":8,"aoû":8,"aoû.":8,septembre:9,sep:9,"sep.":9,sept:9,"sept.":9,octobre:10,oct:10,"oct.":10,novembre:11,nov:11,"nov.":11,"décembre":12,dec:12,"dec.":12,"déc":12,"déc.":12};
