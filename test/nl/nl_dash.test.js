...
test("Test - Should handle nl dayname dd-mm-yy", function() {
...
test("Test - Should handle nl dayname dd - mm - yyyy", function() {
    testSingleCase(chrono.nl, '30 - 12 - 2016  \n', new Date(2012, 7, 9), (result) => {
        expect(result.index).toBe(0);
        expect(result.text).toBe('30 - 12 - 2016');
...
test("Test - Should handle nl dd-mm-yy", function() {
    var text = "30-12-16";
    var results = chrono.nl.parse(text, new Date(2020,4,21));
    expect(results.length).toBe(1)
});

test("Test - Should handle nl dd-mm-yyyy", function() {
    ...
    expect(results[1].text).toBe('03/01/2020');
    expect(results[2].text).toBe('04/01/2020');
