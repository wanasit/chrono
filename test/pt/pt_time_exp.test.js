...
test("Test - Single Expression", function() {
    ...
    var text = "Hospede-se às 6.13 AM";
...
test("Test - Date + Time Expression", function() {
    ...
    var text = "Algo aconteceu em 10 de Agosto de 2012 10:12:59 pm";
    var results = chrono.parse(text, new Date(2012,7,10));
    expect(results.length).toBe(1)
...
test("Test - Time Expression's Meridiem imply", function() {
    ...
    if (result) {
        expect(result.index).toBe(0)
        expect(result.text).toBe('hoje da 1pm às 3')
...
test("Test - Random date + time expression", function() {
    ...
    var text = "quarta 5/1/2013 1115am";
    var result = chrono.parse(text)[0];
    expect(result.text).toBe(text)

    var text = "sexta 5/3/2013 1230pm";
    var result = chrono.parse(text)[0];
    expect(result.text).toBe(text)
    ...
    var text = "segunda 5/13/2013 630-930am";
    var result = chrono.parse(text)[0];
    expect(result.text).toBe(text)

    var text = "quarta 5/15/2013 1030am";
    var result = chrono.parse(text)[0];
    expect(result.text).toBe(text)

    var text = "sexta 6/21/2013 2:30";
    var result = chrono.parse(text)[0];
    expect(result.text).toBe(text)

    var text = "terça 7/2/2013 1-230 pm";
    var result = chrono.parse(text)[0];
    expect(result.text).toBe(text)

    var text = "Segunda, 6/24/2013, 7:00pm - 8:30pm";
    var result = chrono.parse(text)[0];
    expect(result.text).toBe(text)
