...
test("Test - Random text", () => {
    testSingleCase(it.casual, "stasera", new Date(2012, 1 - 1, 1, 12), (result, text) => {
    ...
    testSingleCase(it.casual, "stasera alle 20", new Date(2012, 1 - 1, 1, 12), (result, text) => {
        expect(result.text).toBe(text);
        expect(result.start.get("hour")).toBe(20);
        expect(result.start.get("year")).toBe(2012);
        expect(result.start.get("month")).toBe(1);
    ...
    testSingleCase(it.casual, "stasera alle 8", new Date(2012, 1 - 1, 1, 12), (result, text) => {
        expect(result.text).toBe(text);
        expect(result.start.get("hour")).toBe(20);
        expect(result.start.get("year")).toBe(2012);
        expect(result.start.get("month")).toBe(1);
    ...
    testSingleCase(it.casual, "gio", (result, text) => {
        expect(result.text).toBe(text);
        expect(result.start.get("weekday")).toBe(4);
    });

    testSingleCase(it.casual, "questa sera", new Date(2016, 10 - 1, 1), (result, text) => {
        expect(result.text).toBe(text);
    ...
    testSingleCase(it.casual, "ieri pomeriggio", new Date(2016, 10 - 1, 1), (result, text) => {
        expect(result.text).toBe(text);
        expect(result.start.get("year")).toBe(2016);
    ...
    testSingleCase(it.casual, "domani mattina", new Date(2016, 10 - 1, 1, 8), (result, text) => {
        expect(result.text).toBe(text);
        expect(result.start.get("year")).toBe(2016);
        expect(result.start.get("month")).toBe(10);
    ...
    testSingleCase(it.casual, "questo pomeriggio alle 3", new Date(2016, 10 - 1, 1, 8), (result, text) => {
...
test("Test - Random negative text", () => {
    testUnexpectedResult(it.casual, "nonoggi");

    testUnexpectedResult(it.casual, "xieri");

    testUnexpectedResult(it.casual, "domaniX");
});
